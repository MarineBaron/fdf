...
/*   Updated: 2018/02/27 14:07:51 by mbaron           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

char	*get_value(t_conf *conf, int i)
{
	t_control *c;
...
void		put_control_buttons(t_conf *conf, int y)
{
	t_rect	*rect;

	if (!(rect = get_rect(0, 0, conf->i_btn->w, conf->i_btn->h)))
		set_error("Malloc error in put_control_buttons (get_rect)", 1);
	;
...
void		put_control_values(t_conf *conf)
{
	...
	while (++i < conf->control->nb - 2)
	{
		if (!(value = get_value(conf, i)))
			set_error("Malloc error in put_control_values (ft_itoa)", 1);
